...
exports.sendBitcoins = function sendBitcoins(address, satoshis, fee, callback) {
  ga._sendFrom(pluginConfig.poolAccount, address, satoshis, null, true, function(e, txhash) {
    if (e) {
      console.log('ERROR: sending coins: ' + JSON.stringify(e) + '; Trying without instant.');
      ga._sendFrom(pluginConfig.poolAccount, address, satoshis, null, false, function(e, txhash) {
        if (e) {
          console.log('ERROR: sending coins without instant: ' + JSON.stringify(e) + '; Giving up.');
          callback(e);
