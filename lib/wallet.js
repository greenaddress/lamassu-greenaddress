...
GreenAddress.prototype._sendFrom = function(src, address, satoshis, confirmations, callback) {
  ...
  self.q.push(function(cb) {
    self.conn.call('http://greenaddressit.com/vault/prepare_tx', satoshis, address, 'sender',
...
GreenAddress.prototype.addressReceived = function addressReceived(address, confs, cb) {
  this.conn.call('http://greenaddressit.com/txs/get_balance_by_addr', address).then(function(data) {
...
GreenAddress.prototype.newAddress = function newAddress(account, cb) {
  this.conn.call('http://greenaddressit.com/vault/fund', this.accounts[account]).then(function(script) {
    try {
...
GreenAddress.prototype._sendMany = function(src, addressMap, confirmations, callback) {
  ...
  self.q.push(function(cb) {
    self.conn.call('http://greenaddressit.com/vault/prepare_tx', values, addresses, 'sender',
        {'subaccount': src_num}).then(function(data) {
      try {
        self._sign_and_send_tx(src_num, data).then(function(txid) {
          cb(null, txid);
...
GreenAddress.prototype.makeAccounts = function(cb, additional) {
  var self = this;
  if (!additional) additional = [];
  this.conn.login(this.hdwallet).then(function() {
    async.map(self.DEFAULT_ACCOUNTS.concat(additional),
      function(ac, cb) {
        ...
        self.conn.call('http://greenaddressit.com/txs/create_subaccount', ac[0], ac[1], pk, cc).then(function() {
          self.accounts[ac[1]] = ac[0];
          cb();
        }, function(err) {
...
GreenAddress.prototype._getBalance = function(account, cb) {
  this.conn.call('http://greenaddressit.com/txs/get_balance', this.accounts[account]).then(function(data) {
    cb(null, parseInt(data.satoshi, 10));
  }, function(err) {
    cb(err.desc);
  });
...
GreenAddress.prototype._sign_and_send_tx = function (account, data) {
    ...
    return this.conn.call("http://greenaddressit.com/vault/send_tx", signatures, null);
}
...
GreenAddress.prototype._move = function(src, dst, satoshis, callback) {
  ...
  async.waterfall([
    ...
    function(addr, callback) {
      self.q.push(function(cb) {
        self.conn.call('http://greenaddressit.com/vault/prepare_tx', satoshis, addr, 'recipient',
            {'subaccount': src_num}).then(function(data) {
          try {
