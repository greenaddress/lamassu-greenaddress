...
GreenAddress.prototype._sendFrom = function(src, address, satoshis, confirmations, instant, callback) {
  var self = this;
  ...
  self.q.push(function(cb) {
    self.conn.loginCall('http://greenaddressit.com/vault/prepare_tx', satoshis, address, 'sender',
        {'subaccount': src_num,
         'instant': instant || false}).then(function(data) {
      try {
...
GreenAddress.prototype.sendBitcoins = function sendBitcoins(address, satoshis, transactionFee, cb) {
  var confirmations = 1;
  this._sendFrom(this.poolAccount, address, satoshis, confirmations, false, cb);
};
...
GreenAddress.prototype._accountBalance = function accountBalance(account, confs, cb) {
  this._getBalance(account, confs, cb);
};
...
GreenAddress.prototype.addressReceived = function addressReceived(address, confs, cb) {
  this.conn.loginCall('http://greenaddressit.com/txs/get_balance_by_addr', address, confs).then(function(data) {
...
GreenAddress.prototype._getBalance = function(account, confs, cb) {
  this.conn.loginCall('http://greenaddressit.com/txs/get_balance', this.accounts[account], confs).then(function(data) {
    cb(null, parseInt(data.satoshi, 10));
  }, function(err) {
    cb(err.desc);
  });
